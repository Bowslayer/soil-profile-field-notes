...
function patchApp(html){
  ...
  html=html.replace("x.value=h[k]||'';x.oninput=()=>{h[k]=x.value;save()};", "x.value=h[k]||'';if(words(h.texture)==='sample'&&(k==='texture'||((k==='wetConsistence'||k==='stickiness')&&!x.value)))x.classList.add('sample-pending');x.oninput=()=>{h[k]=x.value;if(k==='texture'){applyTextureDefaults(i,x.value);x.classList.toggle('sample-pending',words(x.value)==='sample')}if((k==='wetConsistence'||k==='stickiness')&&x.value)x.classList.remove('sample-pending');save()};");
